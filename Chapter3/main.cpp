...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (true) {
		...
		//OSからアプリケーション終了メッセージが飛んで来たらループを抜ける
		...
		if(fence_->GetCompletedValue() != fenceVal_) {
			...
			if(event == nullptr) {
				DebugOutputFormatString("CreateEvent failed.\n");
				return -1;
			}
